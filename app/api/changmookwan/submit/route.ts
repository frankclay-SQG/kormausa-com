/**
 * /app/api/changmookwan/submit/route.ts
 *
 * Receives WT Changmookwan intake form data and creates/updates the applicant's
 ...
 * changmookwan_sex (Single-line text)
 * changmookwan_disciplines (Multi-line text / semicolon-separated)
 * changmookwan_application_ref (Single-line text)
 * changmookwan_applied_certs (Single-line text)
 * changmookwan_applied_dan_rank (Number)
 * changmookwan_instructor_level (Number)
 * changmookwan_gym_name (Single-line text)
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (payload.sex) properties.changmookwan_sex = payload.sex;
    if (dobMs) properties.date_of_birth = dobMs;
    if (payload.disciplines?.length)
      properties.changmookwan_disciplines = payload.disciplines.join(';');
    if (payload.ref) properties.changmookwan_application_ref = payload.ref;
    ...
    if (certLabels.length) properties.changmookwan_applied_certs = certLabels.join(';');
    if (payload.appliedRank) properties.changmookwan_applied_dan_rank = payload.appliedRank;
    if (payload.instrLevel) properties.changmookwan_instructor_level = payload.instrLevel;
    if (payload.gymName) properties.changmookwan_gym_name = payload.gymName;
    ...
    console.error('[Changmookwan submit]', message);
