/**
 * /app/api/whmaf/submit/route.ts
 *
 * Receives WHMAF intake form data and creates/updates the applicant's
 ...
 *   whmaf_sex             (Single-line text)
 *   whmaf_disciplines     (Multi-line text / semicolon-separated)
 *   whmaf_application_ref (Single-line text)
 *   whmaf_applied_certs   (Single-line text)
 *   whmaf_applied_dan_rank (Number)
 *   whmaf_instructor_level (Number)
 *   whmaf_gym_name        (Single-line text)
 */
...
// Split "Given FAMILY" → { firstname, lastname }
...
// HubSpot date properties expect Unix ms (midnight UTC)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Build properties — only include defined values
    ...
    if (payload.sex)          properties.whmaf_sex             = payload.sex;
    if (dobMs)                properties.date_of_birth          = dobMs;
    if (payload.disciplines?.length)
                              properties.whmaf_disciplines      = payload.disciplines.join(';');
    if (payload.ref)          properties.whmaf_application_ref  = payload.ref;
    ...
    if (certLabels.length)    properties.whmaf_applied_certs    = certLabels.join(';');
    if (payload.appliedRank)  properties.whmaf_applied_dan_rank = payload.appliedRank;
    if (payload.instrLevel)   properties.whmaf_instructor_level = payload.instrLevel;
    if (payload.gymName)      properties.whmaf_gym_name         = payload.gymName;
    ...
    console.error('[WHMAF submit]', message);
