...
export function Contact() {
  ...
  const fieldClass = "w-full px-4 py-3 rounded-lg bg-white/[0.06] border border-white/10 text-white placeholder-white/25 text-sm focus:outline-none focus:border-korma-gold/50 transition-colors";
  const labelClass = "block text-white/50 text-xs uppercase tracking-wider mb-1.5";
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5 p-8 rounded-2xl border border-white/10 bg-white/[0.03]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className={labelClass}>Full Name *</label><input type="text" name="name" required value={form.name} onChange={handleChange} placeholder="Your full name" className={fieldClass} /></div>
                  <div><label className={labelClass}>Email *</label><input type="email" name="email" required value={form.email} onChange={handleChange} placeholder="your@email.com" className={fieldClass} /></div>
                </div>

                {/* Phone + Interest */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div><label className={labelClass}>Phone</label><input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="(555) 000-0000" className={fieldClass} /></div>
                  <div>
                    <label className={labelClass}>Interested In *</label>
                    <select name="interest" required value={form.interest} onChange={handleChange} className={`${fieldClass} text-white/70 appearance-none`}>
                ...
                <AnimatePresence>
                  {isClass && (
                    <motion.div key="class-fields" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                        <div>
                          <label className={labelClass}>Experience Level</label>
                          <select name="experience" value={form.experience} onChange={handleChange} className={`${fieldClass} text-white/70 appearance-none`}>
                        ...
                        <div>
                          ...
                          <select name="schedule" value={form.schedule} onChange={handleChange} className={`${fieldClass} text-white/70 appearance-none`}>
                ...
                <AnimatePresence>
                  {isSeminar && (
                    <motion.div key="seminar-fields" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="space-y-4 pt-1">
                        {form.interest === "seminar-corporate" && (
                          <div><label className={labelClass}>Organization / Company Name</label><input type="text" name="organization" value={form.organization} onChange={handleChange} placeholder="Your organization" className={fieldClass} /></div>
                        )}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div>
                            <label className={labelClass}>Group Size</label>
                            <select name="groupSize" value={form.groupSize} onChange={handleChange} className={`${fieldClass} text-white/70 appearance-none`}>
                          ...
                          <div><label className={labelClass}>Preferred Date</label><input type="date" name="preferredDate" value={form.preferredDate} onChange={handleChange} className={`${fieldClass} text-white/70`} /></div>
                        </div>
                      </div>
                    </motion.div>
                  )}
                ...
                {/* Message */}
                <div><label className={labelClass}>Message</label><textarea name="message" value={form.message} onChange={handleChange} rows={4} placeholder="Tell us about your goals or ask any questions..." className={`${fieldClass} resize-none`} /></div>
